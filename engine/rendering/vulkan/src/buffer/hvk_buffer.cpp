...
namespace hf
{
    ...
    : memoryType(memoryType), bufferSize(bufferSize)
    {
        ...
        {
            hassert(data, "[Hyperflow] Static buffer is without any data, this is not allowed");
        ...
        case BufferMemoryType::WriteOnly:
        {
            ...
            hvk_assert(vmaMapMemory(GRAPHICS_DATA.allocator, memoryRegions[0], &memoryMappings[0]), "vmaMapMemory Failed!");
        ...
        case BufferMemoryType::PerFrameWriteOnly:
        {
            for (uint32_t i = 0; i < FRAMES_IN_FLIGHT; i++)
            {
                ...
                hvk_assert(vmaMapMemory(GRAPHICS_DATA.allocator, memoryRegions[i], &memoryMappings[i]), "vmaMapMemory Failed!");
            }
        }break;
        default:
            log_fatal("[Hyperflow] Unimplemented buffer memory type");
            abort();
        }
    ...
    uint32_t GetMemoryType(const uint32_t filter, const VkMemoryPropertyFlags props)
    {
        ...
        log_fatal("[Hyperflow] Unable to allocate graphics memory");
        abort();
    }
    ...
    void CreateBuffer(const VkCreateBufferInfo& info, VkBuffer* bufferResult, VmaAllocation* memResult)
    {
        ...
        hvk_assert(vmaCreateBuffer(GRAPHICS_DATA.allocator, &bufferInfo, &vmaAllocInfo, bufferResult, memResult, &resultInfo), "vmaCreateBuffer Failed!");
    }
    ...
    void AllocateImage(const BufferMemoryType memoryType, const VkImage image, VmaAllocation* memResult)
    {
        ...
        default:
            log_fatal("[Hyperflow] Unknown memory type");
            abort();
        }

        VmaAllocationInfo resultInfo{};
        hvk_assert(vmaAllocateMemoryForImage(GRAPHICS_DATA.allocator, image, &vmaAllocInfo, memResult, &resultInfo), "vmaAllocateMemoryForImage Failed!");
        hvk_assert(vmaBindImageMemory(GRAPHICS_DATA.allocator, *memResult, image), "vmaBindImageMemory Failed!");
    }
    ...
    inline void CopyBufferToBuffer(VkCommandBuffer command)
    {
        for (const auto& operation : GRAPHICS_DATA.bufferToBufferCopyOperations)
            vkCmdCopyBuffer(command, operation.srcBuffer, operation.dstBuffer, operation.regionCount, operation.pRegions);
    }
    ...
    {
        void* mapping;
        hvk_assert(vmaMapMemory(GRAPHICS_DATA.allocator, memory, &mapping), "vmaMapMemory Failed!");
    ...
    void BufferOperation(VkCommandBuffer command, VkQueue queue, void (*CopyCallback)(VkCommandBuffer command))
    {
        ...
        hvk_assert(vkResetCommandBuffer(command, 0), "vkResetCommandBuffer Failed!");
        hvk_assert(vkBeginCommandBuffer(command, &beginInfo), "vkBeginCommandBuffer Failed!");

        CopyCallback(command);

        hvk_assert(vkEndCommandBuffer(command), "vkEndCommandBuffer Failed!");
        VkSubmitInfo submitInfo{};
        submitInfo.sType = VK_STRUCTURE_TYPE_SUBMIT_INFO;
        submitInfo.commandBufferCount = 1;
        submitInfo.pCommandBuffers = &command;

        hvk_assert(vkQueueSubmit(queue, 1, &submitInfo, VK_NULL_HANDLE), "vkQueueSubmit Failed!");
        hvk_assert(vkQueueWaitIdle(queue), "vkQueueWaitIdle Failed!");
    }
    ...
    static void SetMemoryTypeFlags(const BufferMemoryType memoryType, VmaAllocationCreateInfo* result)
    {
        ...
        case BufferMemoryType::ReadWrite:
        case BufferMemoryType::PerFrameReadWrite:
        {
            result->usage = VMA_MEMORY_USAGE_AUTO;
